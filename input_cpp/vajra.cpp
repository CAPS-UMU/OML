...
#define INPUT_NUM 3 // Maximum number of inputs (adjustable based on needs)
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2 || argc > 7) {  // 1 to 3 input files
      std::cout << argc << std::endl;
        std::cerr << "Usage: " << argv[0] << " <file_path1> <file_path2> [<file_path3>] <shape1> <shape2> [<shape3>]" << std::endl;
        return 1;
    }

    int inputNum = argc / 2;  // Calculate number of inputs (either 1, 2, or 3)

    std::cout << "inputNum: " << inputNum <<","  << argc << std::endl;

    // Get the file paths and shape strings from the command line arguments
    ...
    // Parse the shape strings into actual tensor shapes
    ...
    // Create the tensors for the input files
...
//    std::cout << "Cycles: " << cycles_after - cycles << std::endl;
    ...
    // Get the result prediction (assuming single output for simplicity)
